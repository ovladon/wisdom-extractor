...
def _heuristic_is_biblio(txt: str) -> bool:
    ...
    bad = [
        r"(?i)\bISBN\b", r"(?i)\bISSN\b", r"(?i)\bp\.\s*\d+\b", r"(?i)\bPages?\b",
        ...
        r"(?i)Infobase Publishing", r"(?i)DeProverbio\.com", r"(?i)WorldCat"
    ]
...
def _has_proverb_cues(txt: str) -> bool:
    ...
    if re.search(r"(?i)\b(if|then|never|always|should|must|avoid|prefer|time|friend|truth|honesty|fortune|blood|work|speak|silence|patience|many|too)\b", txt):
        return True
...
def llm_is_proverb(txt: str, model_path="auto") -> bool:
    ...
    prompt = f"Decide if the following line is a PROVERB (short advice/aphorism) or NOT_PROVERB (bibliography, license, navigation, citation). Return exactly PROVERB or NOT_PROVERB.\n\nLINE: {txt}\nANSWER:"
    try:
