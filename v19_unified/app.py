        ...
        for i, s in enumerate(targets):
            ...
            try:
                ...
                for it in items:
                    ...
                    if insert_proverb(s["id"], it["text"], it["url"], people=people):
                        total_new += 1
...
with tabs[3]:
    ...
    if df.empty:
        ...
        if st.button("🔗 Cluster now"):
            ...
            must = [(c["a_id"], c["b_id"]) for c in list_constraints("must")] if use_constraints else []
            cannot = [(c["a_id"], c["b_id"]) for c in list_constraints("cannot")] if use_constraints else []
...
with tabs[4]:
    ...
    if summary is None or summary.empty:
        ...
        members = df[df["cluster_id"] == cid][["id", "people", "text", "original", "url"]]
...
with tabs[5]:
    ...
    if df.empty:
        ...
        strategy = st.radio("Pair strategy",
                            ["Likely same idea", "Likely different idea", "Surprise me"], horizontal=True)
